...
def run_scheduler(global_config_path: str, local_config_path: str, prom_output_path: str) -> None:
    while True:
        ...
        while not os.path.isfile(global_config_path) or not os.path.isfile(local_config_path):
            if sleep_total == max_wait_time:
                err_msg = 'No global_config: {} or local_config: {}. The scheduler has waited for 2 minutes'.format(
                    global_config_path, local_config_path)
                logger.error('OSError: {}'.format(err_msg))
                raise OSError(err_msg)
            else:
                msg = 'The scheduler is still waiting on the config_service for the global_config or local_config file'
                logger.warning(msg)
                sleep_total += sleep_increment
                time.sleep(sleep_increment)

        # Run linter to transform the local_config file generated by the config_service into objects for the scheduler
        logger.info('Starting linter to transform the config files created by the config_service into python objects')
        ...
        os.makedirs(prom_output_path, exist_ok=True)  # won't make dir if it already exists

        # Run scheduler to asynchronously execute each metric and asynchronously write to the metric's prom file
        scheduler = Scheduler(metrics_list, prom_output_path, logger=logger)
        scheduler.run()
